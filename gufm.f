...
c      open(2,file='neu.dat',status='new')
...
c      print*,' lat (geodetic)?'
      write(*,*) 'Geographische Breite:'
      read(5,*)alat
c     alat =   42.22
c      print*,' long (geodetic)?'
      write(*,*) 'Geographische Laenge:'
      read(5,*)alon
c     alon =  288.94
      alt  =    0.0
c      print*,' start time'
...
c
      write(*,*) '  Jahr    X(nT)      Y(nT)     Z(nT)     H(nT)',
     &'    F(nT)     I(Grad)    D(Grad)'
...
c      write(6,6000)
      write(*,6100) time,x,y,z,h,f,ainc,d
...
6100  format(f8.1,7f10.2)

c      time = time + 1.0
c      if (time .le. time2) then
c        goto 10
c      end if
      stop
      end
...
c      if (lmax.lt.0.or.abs(z).gt.1.d0) pause 'bad arguments'
